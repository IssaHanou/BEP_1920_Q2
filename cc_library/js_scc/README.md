...
- create a class that extends `Device`, in order to do this, implement the following methods:
    ...
    | `performInstruction(action)` | action (a dictionary with optional keys:) | `instruction` (string with the name of the instruction, (special case `time`))                          | boolean of  whether the instruction could be performed                                                                               |
    |                              |                                  | `value` (any type with a value specific for this instruction) (not in case of `time` instruction )               |                                                                                                                                      |
    |                              |                                  | `component_id` (string with the name of the component for which the instruction is meant, this can be undefined) (not in case of `time` instruction )|                                                                                                                                      |
...
### Example
See [these folders](https://github.com/IssaHanou/BEP_1920_Q2/tree/develop-sprint-8/cc_library/example_scripts_js_scc) for 2 examples or see the example below
